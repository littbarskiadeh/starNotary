...
contract("StarNotary", async (accounts) => {
    ...
    describe("star management", () => {
        ...
        it('lets user1 get the funds after the sale', async () => {
            ...
            await starnotary .approve(user2, starId, {from: user1, gasPrice: 0}); 
            await starnotary.buyStar(starId, { from: user2, value: balance });
        ...
        it('lets user2 buy a star and decreases its balance in ether', async () => {
            ...
            await starnotary .approve(user2, starId, {from: user1, gasPrice: 0});  // buyStar() requires that the caller is the owner or approved to transfer that token. 
            await starnotary.buyStar(starId, { from: user2, value: balance, gasPrice: 0 });
        ...
        it('can add the star name {Bigheadphones} and star symbol {BHP} properly', async () => {
            // 1. create a Star with different tokenId ===>  WHAT FOR???
            let starId = 10
            let user1 = accounts[1];
            await starnotary.createStar(starName, starId, { from: user1 });
            //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
            assert.equal(await starnotary.name(), 'Bigheadphones');
            assert.equal(await starnotary.symbol(), 'BHP');
        });
